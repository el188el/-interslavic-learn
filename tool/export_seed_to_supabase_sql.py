#!/usr/bin/env python3
"""
Читает interslavic_learn/assets/data/seed_data.json и генерирует SQL для Supabase:
  INSERT INTO course_categories ...
  INSERT INTO course_lessons ...

Запуск из корня репозитория:
  python tool/export_seed_to_supabase_sql.py

Результат: supabase/seed_import_generated.sql

Дальше: Supabase → SQL Editor → вставить файл (или по частям, если большой) → Run.

Редактор выполняется под правами, обходящими RLS — INSERT в course_* проходит.
"""
...
SEED = ROOT / "interslavic_learn" / "assets" / "data" / "seed_data.json"
OUT = ROOT / "supabase" / "seed_import_generated.sql"
...
def dollar_json(obj: dict) -> str:
    """JSON для ::jsonb без экранирования кавычек."""
...
def main() -> None:
    if not SEED.is_file():
        raise SystemExit(f"Не найден сид: {SEED}")

    with SEED.open(encoding="utf-8") as f:
        data = json.load(f)

    lines: list[str] = [
        "-- Автогенерация: python tool/export_seed_to_supabase_sql.py",
        "BEGIN;",
        "",
    ]
