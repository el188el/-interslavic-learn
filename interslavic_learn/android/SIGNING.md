...
Локально после `flutter build apk --release` файл называется **`app-release.apk`** (`build/app/outputs/flutter-apk/`). Чтобы получить имя **`learn_interslavic_<versionName>.apk`**, переименуйте вручную по образцу в `release/apk_output_name.txt` или скачайте уже переименованный APK из GitHub Actions (шаг workflow после сборки).
...
### Шаг 6a. Supabase в release APK (чтобы работала привязка email и облако)

Без этих секретов сборка всё равно успешна, но в APK **не вшиты** URL и anon-ключ — в приложении будет режим только гостя (как на вашем скриншоте).

1. **New repository secret** → **Name:** `SUPABASE_URL` → **Secret:** URL проекта, например `https://xxxx.supabase.co`.
2. **New repository secret** → **Name:** `SUPABASE_ANON_KEY` → **Secret:** публичный anon key из Supabase **Settings → API** (это не секрет сервера; его обычно вшивают в клиент).

Workflow передаёт их в `flutter build apk` как `--dart-define` (см. `.github/workflows/build_apk.yml`). Локально то же самое: скопируйте значения в `dart_defines.json` или укажите в командной строке (см. `interslavic_learn/dart_defines.example.json`).

### Шаг 7. Проверить список

Для **подписи** APK в CI нужны **эти четыре** имени:
...
Для **облака и входа по email** дополнительно (рекомендуется):

- `SUPABASE_URL`
- `SUPABASE_ANON_KEY`

Имена **чувствительны к регистру** — как в списке.
